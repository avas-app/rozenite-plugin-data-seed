...
async function tokens() {
  ...
  const argWidth = Math.max(
    ...TOKENS.map((entry) => (entry.args ? entry.args.length : 0)),
  )
  ...
  for (const entry of TOKENS) {
    ...
    console.log(
      `  ${entry.token.padEnd(width)}  ${(entry.args ?? '').padEnd(argWidth)}  ` +
        `${entry.summary.padEnd(38)}  ${sampleToken(entry.token)}`,
    )
  }
    ...
    for (const [index, target] of targets.entries()) {
      ...
      try {
        const generator = createGenerator({
          ...
          tsconfig,
          type: typeName,
          // `extended` plus explicit extra tags is what carries `@fake`
          // annotations from the source through to the schema. `faker` is the
          // original spelling and is still read, so no annotation breaks.
          jsDoc: 'extended',
